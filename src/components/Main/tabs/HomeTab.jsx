...
const HomeTab = ({ t, onOpenProfile, user }) => {
  ...
  const handleSurveyComplete = async (surveyId, answers) => {
    try {
      ...
      // Немедленно удаляем завершенный опрос из локального состояния
      setSurveys(prevSurveys => {
        const updatedSurveys = prevSurveys.filter(survey => survey.id !== surveyId);
        console.log(`🗑️ Опрос ${surveyId} удален из локального списка. Осталось опросов: ${updatedSurveys.length}`);
